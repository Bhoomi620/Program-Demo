const myApiData = [
  ...
  const mydisplay = myApiData.map((value,index)=> {
   let myimage = value.Images.map((image,indeximage)=> {
   return (` 
        <button type="button" data-bs-target="#demo${indeximage}" 
        data-bs-slide-to="${indeximage}" class="active"></button>
    `)})

    let myhtmlinnerimages = value.Images.map((myimages,indexinner1)=> {
    return (` 
